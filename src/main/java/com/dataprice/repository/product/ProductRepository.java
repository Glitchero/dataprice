...
public interface ProductRepository extends JpaRepository<Product,String>{
	...
	*/
	 ...
	 * 2.- Products with Matches in SKU
	 * 3.- Filter by category
	 * 4.- Filter by updateDate
	 * @param sellerName
	 * @return
	 */
	
	@Query("select p from Product p where p.seller=:sellerName and p.task.taskName IN :categories and p.updateDay >= :stDate AND p.updateDay <= :edDate and p.upc <> '' and p.upc in (select p.upc from Product p group by p.upc having count(p.upc)> 1)")
	List<Product> getProductsFromSellMatchSkuCatRange(@Param("sellerName") String sellerName, @Param("categories") Set<String> categories , @Param("stDate") Date stDate, @Param("edDate") Date edDate);
